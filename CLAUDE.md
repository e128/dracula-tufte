...
1. **The commit message.** Rationale, measurements, rejected alternatives, and the "do
   not re-fix this" warnings belong here. `git log` and `git blame` are the right place to
   ask why a declaration looks odd.
2. **`backlog.md`** for open decisions and deferred work.
3. **`README.md`** for anything a consumer needs to know.

Never delete a load-bearing explanation outright — move it to one of the three above.
...
Do not "fix" these; they are deliberate and were re-litigated at least once:
