...
| --- | --- |
| v1.48.0 | nothing. Nine more fence types theme correctly with no markup change: `classDiagram`, `stateDiagram-v2`, `erDiagram`, `requirementDiagram`, `kanban`, `radar-beta`, `gitGraph`, `treemap-beta` and Mermaid's own `timeline` keyword (unrelated to this template's `dl.timeline` component; same word, two different things). `gantt` and `architecture-beta` also theme correctly now: both needed one new `themeVariables` entry each (`gridColor`, `archEdgeColor` plus `archGroupBorderColor`), mirrored into `mermaid-palette.json` like every other hex `mermaid.js` carries. `journey` themes its task and section fills the same way but keeps Mermaid's own fixed actor-band and mood-face colors on purpose, see NOTES.md. `zenuml` still does not render: it ships as a second package this template does not import, and stays out on the same cost basis that dropped the separate ELK import in v1.47.0. § 2 gained no new requirement; `accTitle`/`accDescr` already covers every fence. All twelve working types now carry a fixture, in `samples/dark-charts.html` under "More diagram types" (search `classDiagram`) |
