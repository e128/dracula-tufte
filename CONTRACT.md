...
- [ ] A `nav.toc` names itself with `aria-labelledby` pointing at its own visible
...
| --- | --- |
| v1.48.0 | nothing. Nine more fence types theme correctly with no markup change: `classDiagram`, `stateDiagram-v2`, `erDiagram`, `requirementDiagram`, `kanban`, `radar-beta`, `gitGraph`, `treemap-beta` and Mermaid's own `timeline` keyword (unrelated to this template's `dl.timeline` component; same word, two different things). `gantt` and `architecture-beta` also theme correctly now: `gantt` needed one new `themeVariable` (`gridColor`), `architecture-beta` needed two (`archEdgeColor`, `archGroupBorderColor`), all three mirrored into `mermaid-palette.json` like every other hex `mermaid.js` carries. `journey` themes its task and section fills the same way but keeps Mermaid's own fixed actor-band and mood-face colors on purpose, see NOTES.md. `zenuml` still does not render: it ships as a second package this template does not import, and stays out on the same cost basis that dropped the separate ELK import in v1.47.0. § 2 gained no new requirement; `accTitle`/`accDescr` already covers every fence. All twelve working types now carry a fixture, under "More diagram types" (in `samples/dark-charts.html`, search `classDiagram`) |
| v1.47.0 | nothing. `mermaid.js` moves its pinned CDN import from `mermaid@11.17.2` to `mermaid@12.0.0`, self-contained over fences you already emit. Mermaid 12 changed its own defaults, layout to a bundled ELK engine and look to `neo`; this template keeps `look: 'classic'` explicit so rendered diagrams do not gain a drop-shadow, and leaves layout unset so every diagram now lays out with the new ELK default rather than the old `dagre`, which can reflow an existing diagram's geometry on the next render with no markup change on your side. The separate `@mermaid-js/layout-elk` CDN import and its dynamic loader are gone, since v12 bundles ELK into core: nothing to change if you never referenced that pin directly. `usecase-beta` is now a supported fence, styled with the same cluster rules a `systemBoundary` already draws. `themes/` also gained a light variant for five of its non-contract targets (Rider, iTerm2, VS Code, Zed, Ghostty); no submodule reads `themes/`, so this row is informational only |
| v1.46.0 | nothing, unless you emit a `nav.toc`, and then one new § 2 requirement applies: **name the landmark with `aria-labelledby` pointing at its own `p.toc-label`, not with an `aria-label` that repeats the label's text.** § 2 is twenty-five requirements. Everything else in this release is self-contained over markup you already emit, and all of it repairs v1.45.0. The index rule and its list indent were the only physical-side declarations in the stylesheet, so both stayed on the left in RTL while the prose flipped; they are `border-inline-start` and `padding-inline-start` now. The two-column index had no narrow override, and at 320px every entry wrapped to five lines in about 68px of inline space; it is one column under 600px, matching `.col-2`. The print override named `nav.toc` where the `columns` declaration lives on `nav.toc ol`, so a printed index stayed two columns; measured under print emulation before and after. Both `:hover` rules sat outside `@media (hover: hover)`, which leaves a tapped affordance stuck in its hover state on touch, and both moved inside it. The summary disclosure triangles reached the accessibility tree, so `<details>` announced its open state and then a screen reader read "black right-pointing small triangle" on top of it; they take `content: "…" / ""` behind `@supports` now, like the outbound arrow. Two rules set `font-family: var(--sans)` and no token of that name is declared anywhere, so both always resolved to the inherited body serif; the dead declarations are gone and the rendering is unchanged. Both components finally carry a fixture instance, so a mode render and the forced-colors sweep draw them. **One known gap stays open:** the `forced-colors: active` block does not mention `nav.toc`, so the index loses its `color-mix` ground there and reads as a bordered box rather than a filled one |
...
| v1.45.0 | nothing required, and two components became available: **`nav.toc`, an on-this-page index, and `details.deep`, a collapsed tier for detail a reader can skip.** Both are CSS over markup you write yourself, with no script and no CDN. This row was missing until v1.46.0 wrote it, so a consumer who moved the pin to v1.45.1 read § 3, found nothing, and had no way to learn either component existed. The components as shipped in v1.45.0 carried eight defects, every one of them fixed in v1.46.0, so **pin v1.46.0 rather than either v1.45 release if you emit them** |
