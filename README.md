...
| File                  | What It Be |
|-----------------------|------------|
| `tufte-dracula.css`   | **The stylesheet payload.** The complete `<style>…</style>` block (template v1.8.1, oklch palette). Consumers inline this verbatim into every generated HTML file. Includes the wrapping `<style>` tags and 2-space leading indent — the exact byte sequence the renderer emits. |
...
Consumers pin to a tag (currently **`v1.8.1`**) via a git submodule at `external/dracula-tufte/`. To refresh a consumer: bump the submodule pointer, run `git submodule update --remote external/dracula-tufte`, then commit the new pointer.
...
**Click-to-zoom needs no markup.** `mermaid.js` injects a real `<button class="mermaid-zoom">` under each diagram, which be the keyboard and touch path; the overlay is a `role="dialog"` that takes focus, inerts the rest of the body, and returns focus on Escape. The only thing a consumer supplies is the `<div class="mermaid-overlay" id="mermaid-zoom">` as the first child of `<body>`.
...
5. Tag with the next semver: `git tag v1.8.1 && git push origin v1.8.1`.
