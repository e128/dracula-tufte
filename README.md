...
This repo be the canonical home fer the seven artifacts that fer too long were scattered between two repos like spilled grog. Both `pugworks` and `corpus` consume this repo through a pinned git submodule at `external/dracula-tufte/` — one place, one truth, no driftin'.
...
- **[`e128/pugworks`](https://github.com/e128/pugworks)** — `pugworks/src/Pug.Core/Html/CssPathResolver.cs` reads the CSS at `external/dracula-tufte/tufte-dracula.css` at runtime.
- **[`e128/corpus`](https://github.com/e128/corpus)** — `corpus/scripts/html-render.nu` inlines both the CSS and Mermaid script from this submodule at render time.

Both consumers pin to a tag (currently **`v1.5.1`**). To refresh a consumer: bump the submodule pointer, run `git submodule update --remote external/dracula-tufte`, then commit the new pointer.
...
5. Tag with the next semver: `git tag v1.5.2 && git push origin v1.5.2`.
6. Each consumer repo runs `git submodule update --remote external/dracula-tufte` and commits the new pointer.

## Contract Enforcement

The CI workflow `.github/workflows/contract-check.yml` verifies all seven files exist on every push and PR. Consumers' own contract gates (`pug-html-contract-check.nu` on the pugworks side, `data-html-contract-check.nu` on the corpus side) scan their respective repos for any hand-rolled `<style>` blocks that bypass the submodule — those gates fail CI.
