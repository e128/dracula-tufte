...
| File | What it is |
| --- | --- |
| `tufte-dracula.css` | The stylesheet payload (template v1.37.3, oklch palette). The complete `<style>…</style>` block, with its wrapper tags and its leading indent. Consumers inline it verbatim into every generated file. |
...
The current release is **`v1.37.3`**. Consumers reach it through a git submodule. To refresh it,
