...
| `sample.html`         | **Living style fixture.** Generated default-body demo — headings, sidenotes, tables, scorecard, verdict chips, nav, badges, mermaid + zoom, and the markdown-converter set (highlighted code, task list, all five GFM alerts, aligned pipe table, MathML, footnotes). Self-contained. Do not hand-edit — regenerate via `build-sample.nu`. |
...
- **`tabindex="0"` on anything that scrolls sideways**, with a `role="region"` and a label so the stop announces itself. `pre` be `overflow-x: auto`, a table below 600px be its own scroll container, and as of v1.20.0 a `<math display="block">` be one too, so a keyboard user cannot reach the overflowed content otherwise (WCAG 2.1.1). The fixture uses `<pre tabindex="0" role="region" aria-label="Code block">`; the label be a consumer string, so localise it. A converter that emits math owes the same three attributes on any equation wide enough to clip — no stylesheet can add them.
...
- **Syntax highlighting** be styled, not generated — the sheet paints `highlight.js` (`.hljs-*`), `pandoc`/skylighting (`.kw`, `.st`, `.co`, …) and Prism (`.token.*`) classes with the same slot map the editor themes use. Run the highlighter yourself; the sheet colours whatever it emits. Types take a `--purple-bright` token — `--purple` recomputed at `L + 0.07` — because plain purple on `--code-bg` measures 4.23 and misses 1.4.3; in print it be pinned back, where the lift runs the wrong way. Chroma's single-letter classes be deliberately not covered: `.k` / `.s` / `.m` be unnamespaced in a sheet ye inline into yer own pages, and Hugo writes its colours inline by default anyway (`noClasses = true`), where no rule here can reach them.
...
- **Math be styled where it arrives as HTML, and rendered nowhere.** A converter that emits **MathML** (`pandoc --mathml`) needs no script and no CDN: browsers lay it out natively, and the sheet gives a `<math display="block">` its own sideways scroll axis and `pre`'s margin rhythm so a wide equation cannot push the page sideways. A converter that leaves **TeX** as text — `$E = mc^2$`, or a `<span class="math display">` holding it — hands ye literal delimiters until ye load KaTeX or MathJax yerself. That be a pinned CDN and a hard-offline failure of Mermaid's kind, so it stays a consumer decision; the container styling be already here, so KaTeX drops in and inherits the block layout.

**Dark UA furniture be declared, not inherited.** `:root` carries `color-scheme: dark`, and the print block flips it to `light`, so scrollbars, form controls and the UA focus ring render to match the page instead of a light-mode widget dropped onto a dark one. One consequence be unavoidable: GFM emits task-list checkboxes `disabled`, and Chromium ignores `accent-color` on a disabled control, so a ticked box paints UA grey rather than `--purple`. Drawing it ourselves would mean an ungated literal hex in the sheet plus a pseudo-element on an `input`, which WebKit does not honour, so the box stays native — sized to `1em` and spaced off its label either way.
