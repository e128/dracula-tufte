...
| File | What it is |
| --- | --- |
| `tufte-dracula.css` | The stylesheet payload (template v1.34.0, oklch palette). The complete `<style>…</style>` block, with its wrapper tags and its leading indent. Consumers inline it verbatim into every generated file. |
...
The current release is **`v1.34.0`**. Consumers reach it through a git submodule. To refresh it,
...
- **Your own CSS wins with no specificity fight.** The sheet sits in `@layer tufte-dracula`.
  ...
  Load your CSS in any order. The six `!important` declarations go the other way. To beat one of
