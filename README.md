...
| File | What it is |
| --- | --- |
| `tufte-dracula.css` | The stylesheet payload (template v1.29.0, oklch palette). The complete `<style>…</style>` block, with its wrapper tags and its leading indent. Consumers inline it verbatim into every generated file. |
...
The current release is **`v1.29.0`**. Consumers reach it through a git submodule. To refresh it,
